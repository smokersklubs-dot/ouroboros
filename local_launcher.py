# ============================
# Ouroboros — Local launcher (no Google Colab dependency)
# ============================
# Drop-in replacement for colab_launcher.py for local / server installs.
# Reads secrets from environment variables (set by run.sh from .env).
# Stores state in OUROBOROS_DATA_DIR (default: ~/.ouroboros).
...
def ensure_claude_code_cli() -> bool:
    ...
    if subprocess.run(["bash", "-lc", "command -v claude >/dev/null 2>&1"], check=False).returncode == 0:
        return True
    ...
    return subprocess.run(["bash", "-lc", "command -v claude >/dev/null 2>&1"], check=False).returncode == 0


# ----------------------------
# 0.1) apply_patch shim
# ----------------------------
...
def get_cfg(name: str, default: Optional[str] = None, **_kwargs) -> Optional[str]:
    return get_secret(name, default=default)
...
_clean_budget = _re.sub(r'[^0-9.\-]', '', _raw_budget)
try:
...
assert GITHUB_USER and str(GITHUB_USER).strip(), "GITHUB_USER not set — add it to .env"
assert GITHUB_REPO and str(GITHUB_REPO).strip(), "GITHUB_REPO not set — add it to .env"
...
# ----------------------------
_default_data = str(pathlib.Path.home() / ".ouroboros")
DATA_DIR = pathlib.Path(os.environ.get("OUROBOROS_DATA_DIR", _default_data)).expanduser().resolve()

for sub in ["state", "logs", "memory", "index", "locks", "archive"]:
    (DATA_DIR / sub).mkdir(parents=True, exist_ok=True)

DRIVE_ROOT = DATA_DIR  # same variable name as colab_launcher so supervisor modules work unchanged
...
log.info("Data directory: %s", DRIVE_ROOT)
